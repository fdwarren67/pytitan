...
@router.post("/test", tags=["AI Test"])
def test_ai_processing(
    input_text: str = Body(..., description="Natural language input to process"),
    schema_name: str = Body(None, description="Optional specific schema to use"),
    session_id: str = Body(
        None, description="Optional session ID for conversation continuity"
    ),
):
    """
    Test endpoint for AI processing without authentication.
    This endpoint is for development and testing purposes only.
    """
    try:
        result = ai_service.process_input(
            user_input=input_text, schema_name=schema_name, session_id=session_id
        )
...
@router.get("/test/schemas", tags=["AI Test"])
def test_get_schemas():
    """
    Test endpoint to get available schemas without authentication.
    This endpoint is for development and testing purposes only.
    """
    try:
        schemas = ai_service.get_available_schemas()
...
@router.get("/test/conversation/{session_id}", tags=["AI Test"])
def test_get_conversation(session_id: str):
    """
    Test endpoint to get conversation history without authentication.
    This endpoint is for development and testing purposes only.
    """
    try:
...
@router.post("/test/clarify", tags=["AI Test"])
def test_clarify_fields(
    clarification: str = Body(..., description="Clarification for validation errors"),
    session_id: str = Body(..., description="Session ID for conversation continuity"),
):
    """
    Test endpoint for clarification without authentication.
    This endpoint is for development and testing purposes only.
    """
    try:
        result = ai_service.clarify_fields(
            clarification=clarification, session_id=session_id
        )

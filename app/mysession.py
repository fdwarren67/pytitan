...
def issue_tokens(user: Dict[str, Any], roles: List[str]) -> Tuple[str, int, str, int]:
    ...
    refresh_payload = {
        "iss": ISS, "aud": AUD, "iat": iat, "exp": refresh_exp,
        "sub": user["sub"], "email": user.get("email"), "roles": roles,
        "typ": "refresh", "jti": jti,
    }
...
def verify_access(token: str) -> Dict[str, Any]:
    payload = jwt.decode(
        ...
        audience=AUD, options={"require": ["exp", "iat", "aud", "iss"]}
    )
...
def verify_refresh(token: str) -> Dict[str, Any]:
    payload = jwt.decode(
        token, APP_REFRESH_SECRET, algorithms=["HS256"],
        audience=AUD, options={"require": ["exp", "iat", "aud", "iss"]}
    )
